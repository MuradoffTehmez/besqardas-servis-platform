  ...
  route.get("/admin/dispatch", ({ ctx, url }) => {
    ...
    return {
      ...
      technicians: techs.map((t) => {
        ...
        return { id: t.id, fullName: fullName(user), avatarTone: user.avatarTone, employmentType: t.employmentType, specializations: t.specializations.filter((s) => s.status === "ACTIVE").map((s) => specName(s.specializationId)), workload: orders.filter((o) => o.technicianId === t.id && ["CONFIRMED", "IN_PROGRESS", "WAITING_FOR_CUSTOMER"].includes(o.status)).length, rating: t.rating, location: t.location, zoneNames: t.zoneIds.map((z) => db.zones.find((x) => x.id === z)?.name ?? L("—")), city: t.city, workingHours: t.workingHours, events };
      }),
